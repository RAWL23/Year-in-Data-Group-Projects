...
#### MAST5956 SPR: Big Data and Machine Learning (2022/2023) ###################
#### By ########################################################################
#### Aaron Warburton (alw46) | Lauren Rawlings (lar46) #########################
#### Abi Paggett (ap941) | Annabel Robinson (apr24) ############################
################################################################################
...
plot(clean_data$Urbanisation,clean_data$Prevalence_raised_blood_pressure,
     xlab = "Urbanisation", ylab = "Prevalence of raised blood pressure",
...
#This plot presents that the most common systolic blood pressure with the highest frequency is between 126-129 and then gradually decreases after this. 
...
boxplot(Systolic_blood_pressure~Sex, data= clean_data, main="A Box plot showing Systolic Blood Pressure between Sex",
...
plot(clean_data$Urbanisation,clean_data$Systolic_blood_pressure,
     xlab = "Urbanisation", ylab = "Systolic Blood Pressure",
     main = "Scatterplot to show the correlation between 
     Urbanisation and Systolic blood pressure")
...
ggplot(data = scaled_data, mapping = aes(x = Prevalence_morbid_obesity_adults, colour = factor(cluster))) +scale_color_discrete(name = "Cluster")+
  geom_freqpoly(binwidth = 0.005)+labs(x="Prevalence of Morbid Obesity in Adults", y="Frequency", title="Prevalence of Morbid Obesity in Adults by Cluster")
...
#This graph shows us the negative correlations between obese adults and underweight adults. 
#It also shows how different clusters are more frequent in different points on different graphs. 
#So, cluster 2 has a high frequency of low prevalence in the obesity graph but a high fequency of high prevalence in the underweight graph.  
